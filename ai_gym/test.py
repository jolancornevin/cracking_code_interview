...
def select_action(initial_state):
    ...
    if policy.policy_history.dim() != 0:
        policy.policy_history = torch.cat([policy.policy_history, categorical.log_prob(action)])
    else:
        policy.policy_history = (categorical.log_prob(action))
    return action
