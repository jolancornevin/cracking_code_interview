...
class Graph {
    ...
    public:
        ...
        vector<int> shortest_reach(int start) {
            ...
            recursiveBFS(head, &costs, 0);

            // Remove the head from the costs array.
            costs.erase(costs.begin() + start);
            for (int iterCost = 0; iterCost < this->lenNodes - 1; ++iterCost) {
                if (costs[iterCost] == 0) {
                    costs[iterCost] = -1;   
                }
            }
        ...
        void recursiveBFS(
            ...
            costs->insert(costs->begin() + costIndex, previousCost + COST);

            // BFS right first
            if (currentNode->right != NULL) {
                recursiveBFS(currentNode->right, costs, previousCost + COST);
            }
            if (currentNode->left != NULL) {
                recursiveBFS(currentNode->left, costs, previousCost + COST);
            }
        }
