...
def serialize(node):
    ...
    """
    return '[{}, {}, {}]'.format(node.val, serialize(node.right) if node.right else NO_NODE, serialize(node.left) if node.left else NO_NODE)
...
def recursive_deserialize(val, right, left):
...
def deserialize(str):
    ...
    val, right, left = extract_val_from_str(str)
    return recursive_deserialize(val, right, left) 
