...
bool checkNote(vector<string> magazine, vector<string> note) {
    ...
    if (noteSize > magazineSize)
        return false;

    map<string, int> wordMap;
    for (int iterNote = 0; iterNote < noteSize; ++iterNote) {
        wordMap[note[iterNote]] += 1;
    }

    for (int iterMagazine = 0; iterMagazine < magazineSize; ++iterMagazine) {
        if (wordMap[magazine[iterMagazine]] > 0) {
            wordMap[magazine[iterMagazine]]--;
            wordFound++;

            if (wordFound == noteSize)
                return true;
        }
    }
    return false;
